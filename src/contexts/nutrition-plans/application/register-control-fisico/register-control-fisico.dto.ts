...
export class RegisterControlFisicoDto {
  // Cliente al que pertenece el control
  @IsNotEmpty({ message: 'Cliente ID is required' })
...
//   @IsUUID(4, { message: 'Plan ID must be a valid UUID' })
  ...
  // Fecha del control
  @IsNotEmpty({ message: 'Control date is required' })
...
export interface RegisterControlFisicoResponse {
  controlFisico: {
    id: string;
    clienteId: string;
    fechaControl: Date;
